...
Experiments \emph{validate} three theorem-level predictions, not establish ML-benchmark superiority: (i)~detection $>96\%$ below $\sigma^2\beta^2{=}0.25$ (Theorem~\ref{thm:spectral_anomaly}); (ii)~accuracy consistent with the $O(\sigma\beta/\sqrt{T})$ rate (Theorem~\ref{thm:convergence}); (iii)~bounded recovery $T^*$ from corrupted $w^0$ (Theorem~\ref{thm:self_stab}, Fig.~\ref{fig:self_stabilization}).
...
We evaluate three configurations (Table~\ref{tab:configs}) covering a 14$\times$ range of
gradient dimension and distinct heterogeneity-Byzantine profiles.

\begin{table}[htbp]
\centering\small
\caption{Experimental system configurations.}
\label{tab:configs}
\setlength{\tabcolsep}{3pt}
\begin{tabular}{|c|c|c|c|c|l|}
\hline
\textbf{Cfg} & $n$ & $f/n$ & $d$ & $k$ & \textbf{Attack / Notes} \\
\hline
1 & 50 & 40\% & 25M & 256 & Min-max; TV dist 0.68 \\
2 & 32 & 31\% & 22M & 256 & ALIE~\cite{alie_attack}; 8.1GB$\to$260MB \\
3 & 64 & 34\% & 345M & 512 & Grad-inversion; 28GB$\to$890MB \\
\hline
\end{tabular}
\end{table}
...
Table~\ref{tab:detection_results} summarizes detection rates across Byzantine ratios.
Figure~\ref{fig:spectral_fingerprints} (Appendix~\ref{app:experiments}) visualizes the
distinct spectral signatures of each attack type: MinMax and Gaussian create 4 outlier
eigenvalues above $\lambda_+$, while zero-gradient free-riding creates only 2 subtle
outliers, yet all are detected by our KS test.
...
We compare our approach against 11 baseline methods across 12 attack types (144 total experiments: 12 attacks $\times$ 12 aggregators). Table~\ref{tab:accuracy_comparison} reports mean accuracy across all 12 attack types.
...
The ${\sim}15$\,pt.\ gap to FLTrust (63.7\%) and the 58--64\% baseline cluster reflect a
structural floor of pairwise-distance defenses at $\beta=0.4$: Byzantine clients can match
pairwise gradient diversity, but the global eigenvalue structure our filter exploits remains
anomalous (consistent with~\cite{farhadkhani_byzsgd}).
Figure~\ref{fig:convergence} confirms distance-based baselines plateau by round~70--80,
matching the $O(\sigma\beta/\sqrt{T})$ predicted rate (Theorem~\ref{thm:convergence}).
...
\begin{axis}[
    ...
    legend style={font=\scriptsize, draw=black!50, fill=white, fill opacity=0.97, text opacity=1,
      at={(0.5,-0.32)}, anchor=north, legend columns=5, column sep=4pt},
...
recovery time $T^* = O(\Delta_0(\sigma^2\beta^2/\varepsilon^2 + \beta^2/\varepsilon))$ (Assumption~\ref{ass:bounded_init}).}
\label{fig:convergence}
\end{figure}

Phase transition validation (100 trials, $\sigma^2\beta^2 \in [0.01,0.35]$) confirms detection $>96\%$ below the threshold and sharp degradation above it; extended plots are in Appendix~\ref{app:experiments}.
Memory scalability: sketch $k{=}256$ reduces $O(d^2)$ to 262\,MB at $d{=}10^6$; for GPT-2-Medium ($31{\times}$ reduction), see Appendix~\ref{app:ablation}.
