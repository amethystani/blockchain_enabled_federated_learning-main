...
\title{Self-Stabilizing Byzantine-Robust Federated Learning via Spectral Filtering}
...
This paper introduces \emph{our approach}, a Byzantine-robust FL protocol proven self-stabilizing in Dijkstra's framework. The detection mechanism exploits Random Matrix Theory: honest gradients, despite Non-IID heterogeneity, produce eigenspectra following the Marchenko-Pastur (MP) law. Byzantine perturbations create detectable spectral anomalies. The blockchain provides the self-stabilizing \emph{shared memory} substrate: write-once immutability ensures legitimate rounds are permanently recorded, analogous to Dijkstra's shared registers but with Byzantine (not just crash) fault tolerance.

\subsection{Contributions}

The contributions of this work are fivefold:

\textbf{1. Self-Stabilization Proof (distributed systems):} We prove our approach is the first Byzantine-robust FL protocol that is \emph{self-stabilizing} in Dijkstra's sense: from any initial state, it reaches a legitimate configuration in $T^* = O(\sigma f/\varepsilon^2 + f^2/\varepsilon)$ rounds, with closure (once legitimate, stays legitimate). We also prove an impossibility at $\sigma^2 f^2 \geq 0.25$ (Theorem~\ref{thm:phase_impossibility}).
...
\textbf{5. Experimental Validation on Local Blockchain:} We demonstrate end-to-end functionality on a local Hardhat testnet with formal verification of Safety, Liveness, and Self-Stabilization properties, achieving 78.4\% accuracy across 12 attack-aggregator combinations (100\% Byzantine detection rate) versus 63.4\% for the best baseline.
...
\caption{our approach system architecture. Honest and Byzantine clients submit gradients; the aggregator performs spectral detection via the Marchenko-Pastur test; the result is committed on-chain. The blockchain's write-once immutability provides the self-stabilizing shared memory substrate.}
...
We model the federated learning system as a message-passing distributed system in the \emph{synchronous rounds} model~\cite{byzantine_generals}.

\smallskip
\noindent\textbf{Processes.} The system consists of $n$ processes $\mathcal{P} = \{p_1,\ldots,p_n\}$: one \emph{coordinator} (aggregator) and $n-1$ \emph{clients}. Each process has a local state comprising its current model parameters $w^t \in \mathbb{R}^d$.

\smallskip
\noindent\textbf{Communication.} Processes communicate via authenticated point-to-point channels. The coordinator broadcasts $w^t$; clients reply with gradient messages $g_i^t$.

\smallskip
\noindent\textbf{Execution.} An \emph{execution} is an infinite sequence of rounds $t = 0, 1, 2, \ldots$ In each round: (1) coordinator broadcasts $w^t$; (2) each client computes and sends $g_i^t$; (3) coordinator aggregates to produce $w^{t+1}$; (4) aggregated hash is written to the blockchain.

\smallskip
\noindent\textbf{Configurations.} A \emph{configuration} $C = (w, \mathcal{B})$ is a global state comprising the model parameters $w \in \mathbb{R}^d$ and the set $\mathcal{B} \subseteq \mathcal{P}$ of Byzantine clients ($|\mathcal{B}| = f$).

\subsection{Threat Model and Assumptions}
\label{sec:threat_model}

\noindent\textbf{Assumption 1 (Byzantine Fault Model).}
At most $f < n/2$ processes are \emph{Byzantine}: they may deviate arbitrarily from the protocol, send any gradient $g_i^t \in \mathbb{R}^d$, and collude. Byzantine processes have full knowledge of the aggregation algorithm but cannot break cryptographic primitives (hash functions, digital signatures).

\noindent\textbf{Assumption 2 (Honest Gradient Variance).}
Each honest client $i \in \mathcal{H} = \mathcal{P} \setminus \mathcal{B}$ computes $g_i^t = \nabla F_i(w^t) + \xi_i^t$ where stochastic noise $\xi_i^t$ satisfies $\mathbb{E}[(\xi_i^t)_j^2] \leq \sigma^2$ coordinate-wise. We call $(\sigma, f)$ the \emph{heterogeneity-Byzantine profile}.

\noindent\textbf{Assumption 3 (Phase Transition Condition).}
$\sigma^2 f^2 < 0.25$. This necessary condition ensures Byzantine gradients create detectable spectral anomalies (see Theorem~\ref{thm:phase_impossibility}). When violated, no gradient-information-only protocol can distinguish Byzantine from honest clients.

\noindent\textbf{Assumption 4 (Blockchain Liveness).}
The blockchain maintains \emph{liveness}: honest transactions submitted by the majority are eventually finalized. This requires that the blockchain's own BFT consensus~\cite{bft_consensus} tolerates at most $\lfloor(m-1)/3\rfloor$ faulty validators out of $m$ (standard PBFT assumption).

\noindent\textbf{Assumption 5 (Transient Faults).}
Byzantine behavior may occur at any round including round 0 (corrupted initialization). This is the standard \emph{self-stabilization} assumption from Dijkstra~\cite{dijkstra_self_stab}: no assumptions are made on the initial state.

\smallskip
\noindent\textbf{What we do NOT assume:}
\begin{enumerate}[(i)]
\item We do \emph{not} assume Byzantine clients are static; they may change identity across rounds.
\item We do \emph{not} assume a trusted initialization; the initial model may be adversarially corrupted.
\item We do \emph{not} require synchronization beyond the per-round communication pattern.
\end{enumerate}

\smallskip
\noindent\textbf{Attack taxonomy.} Byzantine clients employed in our experiments:
\begin{enumerate}[(i)]
\item \textbf{MinMax~\cite{fl_poisoning}:} Minimizes aggregated gradient norm, maximizing $\|\bar{g}^t\| \to 0$.
\item \textbf{Gaussian:} $g_i = \mathcal{N}(0, c\sigma^2 I)$ with large $c$.
\item \textbf{Label Flip:} Gradients computed on labels shifted by $+1 \pmod{K}$.
\item \textbf{ALIE~\cite{alie_attack}:} $g_i = \bar{g}_{\mathcal{H}} + z \cdot \hat{\sigma}_{\mathcal{H}}$ where $z$ is chosen to stay within statistical tolerance.
\item \textbf{Sign flip:} $g_i = -\alpha \cdot g_{\text{honest}}$.
\item \textbf{Zero gradient:} $g_i = 0$ (free-riding / denial of service).
\end{enumerate}
...
\section{our approach: Theoretical Framework}
...
We establish the convergence guarantee for our approach.

\begin{theorem}[Convergence Rate]
Under the $(\sigma, f)$-threat model, our approach with learning rate $\eta = O(1/\sqrt{T})$ achieves:
\begin{equation}
\min_{t \in [T]} \mathbb{E}[||\nabla F(w^t)||^2] \leq O\left(\frac{\sigma f}{\sqrt{T}} + \frac{f^2}{T}\right)
\end{equation}
...
With maximum delay $\tau_{\text{max}} = 10$ rounds, our approach maintains convergence rate $O(\sigma f/\sqrt{T} + f^2/T)$ with detection rate degrading by at most 12\% compared to synchronous aggregation.
\end{theorem}
...
We evaluate our approach across three deployment scales using naturally partitioned data and production-realistic threat models:
...
We compare our approach against 11 baseline methods across 12 attack types on the medium-scale setup (144 total experiments: 12 attacks $\times$ 12 aggregators including our approach). Results shown in Table~\ref{tab:accuracy_comparison} report mean accuracy averaged across all 12 attack types.
...
our approach achieves 15 percentage point improvement over the best baseline (FLTrust) and 30 percentage points over FedAvg. our approach wins on all 12 attack types, achieving best performance across every attack scenario tested.
...
\legend{Clean (No Attack), \textbf{our approach (Ours)}, FLTrust, Krum, FedAvg}
\end{axis}
\end{tikzpicture}
\caption{Convergence under Byzantine attacks (40\% adversarial clients): our approach achieves 79.1\% accuracy vs 63.4\% for best baseline (FLTrust), approaching clean performance of 85.0\%.}
...
\begin{axis}[
    ...
    legend style={font=\small, draw=black!50, fill=white, fill opacity=0.95, text opacity=1, cells={anchor=west}, at={(0.01,0.99)}, anchor=north west},
    tick label style={font=\small},
    xlabel style={font=\normalsize},
    ylabel style={font=\normalsize},
    minor tick num=1
...
\draw[line width=2.5pt, red!60!black, densely dashed] (axis cs:0.25,0) -- (axis cs:0.25,115);
\node[red!60!black, font=\small\bfseries, fill=white, inner sep=2pt, draw=red!60!black, rounded corners] at (axis cs:0.25,108) {$\sigma^2 f^2{=}0.25$};

% Region labels
\node[blue!70!black, font=\small\bfseries, fill=white, inner sep=2pt, rounded corners, opacity=0.9] at (axis cs:0.10,20) {Detectable};
\node[orange!80!black, font=\small\bfseries, fill=white, inner sep=2pt, rounded corners, opacity=0.9] at (axis cs:0.31,20) {Undetectable};
...
\begin{axis}[
    ...
    height=0.5\textwidth,
    legend pos=north west,
    legend style={font=\small, draw=black!50, fill=white, fill opacity=0.95, text opacity=1, at={(0.02,0.98)}, anchor=north west},
    tick label style={font=\small},
    xlabel style={font=\normalsize},
    ylabel style={font=\normalsize},
    xmin=5e4, xmax=2e9,
...
our approach provides data-dependent certificates that adapt to observed heterogeneity. Table~\ref{tab:certified_comparison} compares certified robustness guarantees.
...
\hline
\textbf{our approach} & \textbf{38\%} & Data-dependent ($\sigma^2 f^2 < 0.25$) \\
...
our approach provides 2.5$\times$ stronger certificates (38\% vs 15\% Byzantine tolerance) by adapting to observed data heterogeneity rather than using fixed norm bounds.
...
\textbf{Medium-Scale Results:} On Federated EMNIST with ResNet-50 under min-max attacks (40\% Byzantine), our approach achieves 82.4\% accuracy versus 55.7\% for FLTrust, 63.2\% for FLAME, 60.8\% for Bulyan++, and 74.1\% for SignGuard. The clean baseline achieves 84.9\%. The overall average across all 12 attack types is 78.4\% (see Table~\ref{tab:accuracy_comparison}). Against adaptive spectral-aware attacks calibrated to our detection threshold, we maintain 78.1\% accuracy by combining spectral filtering with gradient clipping ($\sigma=0.15$).

\textbf{Large-Scale Results:} On Tiny ImageNet with ViT-Small, our approach achieves 76.3\% top-1 accuracy versus 81.7\% clean and 58.4\% best baseline (FLTrust). Wall-clock overhead is 6.8s per round vs. 3.2s baseline aggregation, 52\% faster than geometric median (14.7s). Network transfer is 34MB per node vs. 89MB for full gradient exchange. Our sketched implementation (k=256) uses 260MB memory vs. 8.1GB full covariance.

\textbf{Foundation Model Results:} Fine-tuning GPT-2-Medium on WikiText-103, our approach maintains perplexity 24.3 vs. 21.1 clean and 52.8+ for all baselines. Layer-wise sketching uses 890MB memory vs. 28GB full covariance. Critically, we demonstrate robustness on decoder-only architectures where attention layer gradients have rank-deficient structure that breaks standard robust aggregation assumptions.
...
We formalize our approach as a self-stabilizing distributed system in the sense of Dijkstra~\cite{dijkstra_self_stab}. A distributed system is \emph{self-stabilizing} if, regardless of the initial configuration, it eventually reaches and maintains a legitimate configuration.

\begin{definition}[Legitimate Configuration]
A system configuration $(w^t, \mathcal{H}^t)$ at round $t$ is \emph{legitimate} iff:
\begin{enumerate}[(i)]
\item All Byzantine clients are correctly identified: $\mathcal{B}^t = \mathcal{B}$ (true Byzantine set)
\item Aggregated gradient approximates honest mean: $\|\hat{g}^t - \bar{g}^t\|^2 \leq \varepsilon_{\text{agg}}$
\item On-chain round state is consistent: round $t$ finalized with correct hash
\end{enumerate}
\end{definition}

\begin{theorem}[Self-Stabilization of our approach]
\label{thm:self_stab}
Under the $(\sigma, f)$-Byzantine threat model with $\sigma^2 f^2 < 0.25$, the our approach + Blockchain protocol $\Pi$ is self-stabilizing with:
\begin{itemize}
...
We establish self-stabilization via four properties:

\textbf{(i) Detection Completeness:} When $\sigma^2 f^2 < 0.25$, Byzantine gradients create eigenvalue anomalies beyond $\lambda_+ = \sigma^2(1+\sqrt{\gamma})^2$. By Theorem~\ref{thm:spectral_anomaly}, these are detectable with probability $\geq 1 - e^{-k/\log^2 k}$, ensuring Byzantine clients are identified within 1 round.

\textbf{(ii) Aggregation Correctness:} Post-detection, honest gradients satisfy $\|\hat{g}^t - \bar{g}^t\|^2 \leq O(\sigma^2 f^2)$ by concentration inequalities, establishing legitimate configuration property (ii).

\textbf{(iii) Blockchain Immutability (Stabilizing Medium):} The blockchain provides a shared-memory substrate equivalent to Dijkstra's shared registers~\cite{dijkstra_self_stab}, but with Byzantine fault tolerance (vs.\ crash faults). Even with $f < n/2$ Byzantine nodes, the honest majority's view of completed rounds is preserved on-chain. Progress toward $w^*$ cannot be undone.

\textbf{(iv) Convergence Under Correction:} With $\eta = O(1/\sqrt{T})$:
\begin{equation}
F(w^{t+1}) \leq F(w^t) - \eta\|\nabla F(w^t)\|^2 + \eta^2 O(\sigma^2 f^2)
\end{equation}
Telescoping over $T^* = O(\sigma f/\varepsilon^2)$ rounds yields $\min_t \|\nabla F(w^t)\|^2 \leq \varepsilon^2$. \hfill$\square$
\end{proof}

\subsection{Blockchain as Self-Stabilizing Shared Memory}

Classical self-stabilizing algorithms~\cite{dijkstra_self_stab,dolev_self_stab} assume shared registers accessible to all processes. Our blockchain provides a stronger substrate:

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{fig_blockchain_stabilizer.png}
\caption{Blockchain as self-stabilizing shared memory. (a) Timeline showing monotonic progress: each finalized round is permanently recorded on-chain, preventing Byzantine rollback. (b) Formal property comparison between classical shared memory and blockchain-based stabilization, highlighting the stronger guarantees provided by Byzantine fault-tolerant consensus.}
...
\begin{tabular}{|l|c|c|}
\hline
\textbf{Property} & \textbf{Classical Shared Mem.} & \textbf{Blockchain} \\
\hline
Fault model & Crash ($f < n/2$) & Byzantine ($f < n/2$) \\
Read access & Any process & Any process \\
...
The blockchain's immutability property provides a stronger stabilization guarantee than classical shared memory: once a legitimate round is finalized on-chain, it cannot be overwritten even by Byzantine nodes. This ``write-once'' semantic is the key innovation enabling self-stabilization with Byzantine (not just crash) faults.

\begin{lemma}[Closure via Blockchain Immutability]
\label{lem:closure}
Once our approach enters a legitimate configuration at round $t_0$, it remains legitimate for all $t \geq t_0$ with probability $1 - \delta$, where $\delta = O(e^{-k/\log^2 k})$ per round.
\end{lemma}

\begin{proof}
In a legitimate configuration: (i) all Byzantine clients are detected and excluded; (ii) the on-chain record of round $t_0$ is immutable (BFT consensus with $f < n/2$ ensures no rollback); (iii) future Byzantine clients are detected within 1 round by Property (i) of Theorem~\ref{thm:self_stab}. The only failure mode---Byzantine clients constructing undetectable gradients---occurs with probability $\leq \delta$ by the MP anomaly detection bound. Hence the closure condition holds with probability $\geq 1 - \delta$ per round. For $T$ rounds, union bound gives $1 - T\delta$, which approaches 1 for $k = \omega(\log^3 T)$. \hfill$\square$
\end{proof}
...
\begin{remark}[Comparison with FLP]
FLP impossibility requires $\geq 1$ crash fault; our impossibility requires $\sigma^2 f^2 \geq 0.25$. Both characterize fundamental barriers that cannot be overcome without additional assumptions (randomization for FLP; trusted validation sets or additional information for spectral detection).
\end{remark}
