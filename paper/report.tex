...
For $\sigma^2 f^2 \geq 0.25$, no Byzantine-resilient FL protocol using only gradient information can be self-stabilizing: Byzantine clients can always construct gradients statistically identical to honest heterogeneous gradients, preventing any algorithm from distinguishing them.
\end{theorem}

This is analogous to the FLP impossibility~\cite{flp_impossibility} in distributed consensus: just as asynchrony + crash faults prevent deterministic consensus, high heterogeneity + Byzantine faults prevent spectral detection. The $\sigma^2 f^2 = 0.25$ boundary is a new impossibility result specific to Byzantine-robust federated learning.

\subsection{Experimental Validation of Self-Stabilization}

Figure~\ref{fig:self_stab_recovery} demonstrates recovery from arbitrary initial states. Starting from corrupted models $\|w^0 - w^*\| \in \{0.1, 1.0, 10.0, 50.0\}$, Spectral Sentinel stabilizes within a bounded number of rounds $T^*$ that is \emph{independent of initial corruption magnitude}---the defining property of self-stabilization.

The blockchain's role is critical: without on-chain coordination, Byzantine nodes could prevent stabilization by repeatedly corrupting rounds. With blockchain immutability, each completed honest round is permanently recorded, ensuring monotonic progress toward $w^*$ regardless of Byzantine interference.
