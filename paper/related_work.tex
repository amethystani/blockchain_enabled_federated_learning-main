...
All prior Byzantine-robust FL methods~\cite{krum,bulyan,crfl,flame,byzshield} guarantee
convergence from a \emph{known-good} initialization; none provide a formal recovery time
bound from arbitrary corrupted states.
We provide $T^* = O(\sigma^2\beta^2/\varepsilon^2 + \beta^2/\varepsilon)$, satisfying
the practical self-stabilization definition of~\cite{dolev_self_stab}.
\textbf{(ii) Spectral vs.\ pairwise distance:}
Krum, Trimmed Mean, Bulyan, and FLTrust operate on pairwise or coordinate-wise distances,
which degrade at $\beta > 1/3$ because Byzantine clients can mimic honest pairwise
diversity.
Our filter operates on the \emph{global eigenvalue distribution} via the BBP phase
transition~\cite{bbp_transition}, remaining effective up to $\beta < 1/2$.
\textbf{(iii) Blockchain as stabilizing medium vs.\ auditability:}
Prior blockchain FL~\cite{blockchain_fl,blockchain_smart} targets auditability.
We use write-once BFT consensus as the Byzantine-fault-tolerant analog of Dijkstra's
shared registers~\cite{dijkstra_self_stab}, enabling the closure argument
(Lemma~\ref{lem:closure}) that classical shared memory cannot support under Byzantine faults.
The closest SSS precedents~\cite{sss_byz_broadcast,sss_byz_counter,sss_blockchain_sharding}
provide distributed safety properties but do not address self-stabilization of a learning
protocol.
