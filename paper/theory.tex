...
\textbf{(a)} Eigenvalue histograms for honest-only gradients (blue, confined within MP
bulk) vs.\ a Byzantine attack (red, with outlier spikes above $\lambda_+$, dashed).
The Marchenko-Pastur upper edge $\lambda_+ = \hat{\sigma}^2(1+\sqrt{\hat{\gamma}})^2$
(orange) cleanly separates Byzantine outliers from the honest bulk.
\textbf{(b)} Empirical detection rate (orange dots) and theoretical bound (blue curve)
as a function of the heterogeneity metric $\sigma^2\beta^2$.
Below the phase transition ($\sigma^2\beta^2 < 0.25$, green region), detection
exceeds 96\%; beyond it (red region), Byzantine gradients become spectrally
indistinguishable from honest heterogeneity---a fundamental impossibility
